...
\added{Unlike the high- and intermediate-frequency cases, we find that the groups of pixels most consistent with low-frequency heating do not appear as spatially-coherent loop-like structures, but instead have a ``patchy'' appearance.
...
\added{Additionally, we note that one could, in principle, model an entire active region with only steady heating and still reproduce the distribution of observed emission measure slopes.
...
\added{The relative importance of features from these three channels is again consistent with \citet{bradshaw_patterns_2016} who found that short, but non-zero 211-193 \AA{} time lags, in combination with the prevalence of zero 171-131 \AA{} time lags, are inconsistent with low-frequency nanoflares.}

\added{Interestingly, we find that there are no channel pairs, either for the time lag or maximum cross-correlation, that include the 94 \AA{} channel in the ten most important features as shown in \autoref{tab:importance}.    
