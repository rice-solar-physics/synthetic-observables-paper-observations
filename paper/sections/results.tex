...
Random forest classifiers, first developed by \added{\citet{ho_random_1995} and later improved by} \citet{breiman_random_2001}, provide an ensemble statistical learning method for combining many noisy, decorrelated decision trees in order to improve prediction accuracy and robustness.
...
\added{While \citetalias{barnes_understanding_2019} only discussed 31 synthetic features (15 time lags, 15 maximum cross-correlation values, 1 emission measure slope), here we have calculated $T_{peak}$ for our synthetic emission measure distributions as well such that we have 32 features in our synthetic data set and 32 in our observational data set.}
...
\added{The class probability in each pixel provides a measure of the confidence of the classifier in assigning a given heating frequency label to that pixel.
If one heating frequency has a class probability close to 1, the other two classes will be, by definition, close to 0, and the confidence of the classification is high.
If the class probability for each heating frequency is $1/3$, the classifier is not able to distinguish which heating frequency is most probable in that pixel.}
