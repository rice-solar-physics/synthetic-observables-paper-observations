...
Note that if $G_{m,R}=G_{m,L}=G_m$, $\Delta G_m=0$ because the split at node $m$ did not do a better job of discriminating between classes than the split at the previous node. However, if $G_{m,R}$ or $G_{m,L}$ decreases such that purity of the left or right node increases, $\Delta G_m > 0$ because the split at node $m$ has added information to the classifier by preferentially sorting samples of a single class to either the left or right child node.
