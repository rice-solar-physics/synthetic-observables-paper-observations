...
The \AR{} was chosen from the catalogue of \AR s originally compiled by \citet{warren_systematic_2012} and was also studied by \citetalias{viall_survey_2017}.
...
After prepping, aligning, and cropping all 12 hours of AIA data for all six channels, we carry out the same analysis that we applied to our predicted observations in \citetalias{barnes_understanding_2019} in order to compute two diagnostics of the heating: the emission measure slope and the time lag.
...
\added{The inner core (``IC'') and the periphery (``P'') of the \AR{} are indicated in red in the left panel of \autoref{fig:em-slopes}.
...
We define the periphery as the region farthest from the inner core containing closed loops which are still visible relative to the surrounding diffuse emission.}
The emission measure slope tends to be more steep near the center of the \AR{} and tends to increase from $\sim2.5$ to $>5$ as we move from the periphery to the inner core of the \AR{}.
...
\added{Following the convention of \citet{viall_evidence_2012}, we take the order of the channels, from hottest to coolest, to be: 94, 335, 211, 193, 171, 131 \AA{}, meaning that \textit{a positive time lag indicates cooling plasma}.
Observationally, this is often a good representation of how the plasma in a quiescent \AR evolves through the AIA channels, though not always.}
...
\added{The method employed here for calculating the time lag \citepalias[see Appendix C of][]{barnes_understanding_2019} is quantitatively identical to the method used by \citetalias{viall_survey_2017} \citep[see Section 2 of][]{viall_evidence_2012}.
...
\explain{Looking closely at the histograms for the 211-193 pair, I see mostly zeros and in fact close to the same number of zeros as the 171-131 case. I looked back at the \citet{viall_survey_2017} paper and there does not appear to be a map for this channel pair there. In \citet{viall_evidence_2012}, I see that there are maps for both though they are on modified color tables to make those short time lags stand out.}
...
\added{In this case, the cooler, 1 MK component of the 94 \AA{} dominates the emission.}
...
\added{This is indicative of an excess of hot plasma in our model \AR{} relative to the observations.}
...
\explain{I went back and looked at the distributions of max cross-correlation values for each channel pair an compared them to those shown in \citet{viall_evidence_2012} and they are all qualitatively similar.}

In every channel pair, we find that the maximum cross-correlation \replaced{is highly structured}{maps reveal coherent loop-like structures similar to those seen in the observed intensities shown in \autoref{fig:intensity-maps}}, indicating that these loops and the surrounding diffuse emission are evolving coherently through the AIA passbands.
...
\added{In general, this is expected as the periphery has less emission than the core in all channels and thus lower count rates.
